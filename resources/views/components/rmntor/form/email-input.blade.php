<div class="flex flex-col-reverse">
    <span class="peer has-[:focus]:border-b-2 text-input border border-black/20 has-[:placeholder-shown]:has-[:invalid]:border-black/20 has-[:invalid]:border-danger text-sm flex items-center rounded-[12px] {{$class}}">
        ...
        <input wire:model='{{$name}}' id="input-email-{{$name}}" name="{{$name}}" type="email" class="m-2 outline-none w-full" placeholder="{{$placeholder}}" required>
    </span>
    <label for="input-email-{{$name}}" class="mb-1 block text-sm font-medium peer-has-[:focus]:text-main peer-has-[:valid]:text-main">{{$label}}</label>
</div>
